...
#It will also add the Turbonomic Dev Service Principal to the Turbonomic custom role scoped to the new storage account(s)
...
foreach($storloc in $vmsloc){
    ...
    if(($turboCustomRole = Get-AzureRmRoleDefinition -Name 'Turbonomic Operator ReadOnly') -eq $null){
        ...
        if(($turboCustomRole = Get-AzureRmRoleDefinition -Name 'Turbonomic Operator ReadOnly') -eq $null){
            ...
            Start-Sleep 30
            $selectSub = Select-AzureRmSubscription -Subscription $subscriptionId
            $turboSPNlist = get-azurermadserviceprincipal | where-object{$_.DisplayName -eq 'Turbonomic_Dev'}
