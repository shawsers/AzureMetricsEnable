...
#Comment this line out below if running the script from Azure Cloud Shell
...
foreach ($azuresub in $sname){
    ...
    #First some checking on existing Log Analytics workspaces in the subscription
    write-host "First checking to see if any Log Analytics workspaces already exist" -ForegroundColor Blue
    $checklaw = Get-AzOperationalInsightsWorkspace | select "Name", "Location", "ResourceGroupName"
    if ($checklaw -eq $null){
      write-host "No Log Analytics Workspaces found.....procedding" -ForegroundColor Green
      Continue
    } else {
      write-host "Here is a list of the existing Log Analytics Workspaces found...." -ForegroundColor Green
      $checklaw
    }
    write-host ""
