...
if (($valres = Get-AzureRmResourceGroup -Name $resourcegroup -ErrorAction SilentlyContinue) -eq $null){
    ...
    $newresgroup = New-AzurermResourceGroup -Name $resourcegroup -Location $location -tag @{"ghs-solution"="turbonomic_pwcit"; "ghs-environment"="turbonomic_pwcit_prod"; "ghs-los"="ifs"; "ghs-appid"="hycsapp1883"; "ghs-owner"="jerry.trollo@pwc.com"; "ghs-tariff"="zae"; "ghs-apptioid"="globalnis645"} -ErrorAction Stop
} else {
...
foreach($storloc in $vmsloc){
 ...
 #   $storageaccountname = $storageaccount + $count
    ...
    if ($getStorage -eq $null){
        ...
        $selectSub = Select-AzureRmSubscription -Subscription $subscriptionId
        $error.clear()
        $newStorage = New-AzurermStorageAccount -ResourceGroupName $resourcegroup -Name $storageaccountname -Location $storloc -Kind StorageV2 -SkuName Standard_LRS -EnableHttpsTrafficOnly $true -tag @{"ghs-solution"="turbonomic_pwcit"; "ghs-environment"="turbonomic_pwcit_prod"; "ghs-los"="ifs"; "ghs-appid"="hycsapp1883"; "ghs-owner"="jerry.trollo@pwc.com"; "ghs-tariff"="zae"; "ghs-apptioid"="globalnis645"} -ErrorAction Stop
